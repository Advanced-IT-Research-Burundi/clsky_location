                    ...
                    <div class="card-body">
                        <form action="{{ route('payments.store') }}" method="POST">
                            ...
                            <div class="mb-3">
                                ...
                                <select class="form-select @error('reservaion_id') is-invalid @enderror" id="reservation_id"
                                    name="reservation_id" required>
                                    <option value="">Sélectionner une réservation</option>
                                    @foreach ($reservations ?? [] as $reservation)
                                        <option value="{{ $reservation->id }}" data-amount="{{ $reservation->total_price }}"
                                            {{ old('reservation_id') == $reservation->id ? 'selected' : '' }}>
                                            #{{ $reservation->id }} -
                                            {{ $reservation->property?->title ?? 'Propriété supprimée' }}
